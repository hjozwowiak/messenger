defmodule Auth.UserTest do
  ...
  describe "create/1" do
    ...
    test "returns error when email has an incorrect format" do
      ...
      assert {:error, result} = Subject.create(params)
      assert "Incorrect email format." in errors_on(result).email
    end
    ...
    test "returns error when password is too weak" do
      ...
      assert "Password should contain a minimum of eight characters, at least one uppercase, one lowercase, one number and one special character." in errors_on(
               result
             ).password
    end
  end
